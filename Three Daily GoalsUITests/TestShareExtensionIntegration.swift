    ...
    func testMultipleShareOperations() async throws {
        ...
        for (index, operation) in shareOperations.enumerated() {
            ...
            if let task = try await processShareWorkflow(provider: mockProvider) {
                print(
                    "DEBUG: Created task \(index): title='\(task.title)', details='\(task.details)', url='\(task.url ?? "nil")'"
                )
...
class MockNSItemProvider: NSItemProvider {
    ...
    override func loadItem(forTypeIdentifier typeIdentifier: String, options: [AnyHashable: Any]? = nil) async throws
        -> NSSecureCoding
    {
        ...
        default:
            throw NSError(
                ...
                userInfo: [NSLocalizedDescriptionKey: "Unsupported type: \(typeIdentifier)"])
        }
    }

    func loadItem(
        forTypeIdentifier typeIdentifier: String, options: [AnyHashable: Any]? = nil,
    ...
    func loadDataRepresentation(forTypeIdentifier typeIdentifier: String) async throws -> Data {
        ...
        case UTType.plainText.identifier, UTType.text.identifier:
            ...
            throw NSError(
                domain: "MockError", code: -1, userInfo: [NSLocalizedDescriptionKey: "No text data available"])
        case UTType.url.identifier:
            if let url = mockURL {
        ...
        case UTType.fileURL.identifier:
            ...
            throw NSError(
                domain: "MockError", code: -1, userInfo: [NSLocalizedDescriptionKey: "No file URL data available"])
        default:
            throw NSError(
                domain: "MockError", code: -1,
                userInfo: [NSLocalizedDescriptionKey: "Unsupported type: \(typeIdentifier)"])
