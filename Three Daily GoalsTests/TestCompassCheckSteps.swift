...
struct TestCompassCheckSteps {
    ...
    private func createTestPreferencesWithPlanEnabled() -> CloudPreferences {
        ...
        assert(
            testPreferences.isCompassCheckStepEnabled(stepId: "plan") == true,
            "Plan step should be enabled in test setup")
        assert(
            testPreferences.isCompassCheckStepEnabled(stepId: "inform") == true,
            "Inform step should be enabled by default")
    ...
    func testReviewStep() throws {
        let appComponents = setupApp(
            isTesting: true, loaderForTests: createTestDataLoader(), preferences: createTestPreferencesWithPlanEnabled()
        )
        let dataManager = appComponents.dataManager
        let timeProvider = appComponents.timeProvider
        let step = ReviewStep()
    ...
    func testStepManagerFlow() throws {
        ...
        // Should move from review to plan (on macOS, after MoveToGraveyard)
        compassCheckManager.moveStateForward()
    ...
    func testStepManagerFlowWithEmptyData() throws {
        ...
        let appComponents = setupApp(
            isTesting: true, loaderForTests: createEmptyDataLoader(),
            preferences: createTestPreferencesWithPlanEnabled(), compassCheckSteps: stepsWithControlledSkipping)
        let compassCheckManager = appComponents.compassCheckManager
    ...
    func testStepManagerFlowWithOnlyPriorityTasks() throws {
        ...
        let appComponents = setupApp(
            isTesting: true, loaderForTests: createPriorityOnlyDataLoader(), compassCheckSteps: priorityFocusedSteps)
        let compassCheckManager = appComponents.compassCheckManager
    ...
    func testStepManagerMoveToNextStep() throws {
        ...
        // Test moving from currentPriorities to next step (EnergyEffortMatrix)
    ...
    func testCompleteCompassCheckFlow() throws {
        ...
        let appComponents = setupApp(
            isTesting: true, loaderForTests: createTestDataLoader(),
            preferences: createTestPreferencesWithPlanEnabled(), compassCheckSteps: simplifiedSteps)
        let compassCheckManager = appComponents.compassCheckManager
        let dataManager = appComponents.dataManager
    ...
    func testIOSFlow() throws {
        ...
        let appComponents = setupApp(
            isTesting: true, loaderForTests: createTestDataLoader(), compassCheckSteps: iOSSteps)
        let compassCheckManager = appComponents.compassCheckManager
    ...
    func testMacOSFlow() throws {
        ...
        let appComponents = setupApp(
            isTesting: true, loaderForTests: createTestDataLoader(),
            preferences: createTestPreferencesWithPlanEnabled(), compassCheckSteps: macOSSteps)
        let compassCheckManager = appComponents.compassCheckManager
    ...
    func testMoveToGraveyardStepInCompassCheckFlow() throws {
        ...
        let testDataLoader = createTestDataLoader()
        let appComponents = setupApp(
            isTesting: true, loaderForTests: testDataLoader, preferences: testPreferences,
            compassCheckSteps: focusedSteps)
        let timeProvider = appComponents.timeProvider
